...
describe('Create Car Specification Controller', () => {
  ...
  it('should be able to create a new car specification', async () => {
    ...
    const {
      body: [specification],
    } = await request(app).get('/specifications');
    ...
    expect(response.body.specifications.length).toEqual(1);
    expect(response.body.specifications[0].id).toEqual(specification.id);
  });
});
